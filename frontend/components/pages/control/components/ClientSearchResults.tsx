...
export default function ClientSearchResults({
  ...
  const getSortIndicator = (key: string) => {
    if (sortConfig.key !== key) {
      return <span className="ml-1.5 opacity-30 group-hover:opacity-100 transition-opacity">↕</span>;
    }
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-white dark:bg-gray-950 border border-indigo-100 dark:border-indigo-900/30 rounded-xl shadow-xs mt-6">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-4">Buscando clientes...</p>
      </div>
    );
  }
  ...
  if (results.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 bg-white dark:bg-gray-950 border border-indigo-100 dark:border-indigo-900/30 rounded-xl shadow-xs mt-6 text-center">
        ...
        <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200">No se encontraron clientes</h3>
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 max-w-sm">
          Ajusta los filtros de búsqueda y presiona <strong>Buscar</strong> para intentar de nuevo.
        </p>
      </div>
    );
  }
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-indigo-50/50 dark:divide-indigo-900/10">
            {sortedData.map((client) => {
              const fullNames = `${client.apellidos || ""} ${client.nombres || ""}`.trim().toUpperCase();

              const plansText = client.plans && client.plans.length > 0
                ? client.plans.map((p) => p.plan).join(", ")
                : "Sin Plan";

              const phoneText = client.contact?.telephone || client.contact?.phoneSms || client.contact?.phoneTwo || "N/A";
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-xs text-gray-600 dark:text-gray-300 max-w-[200px] truncate" title={plansText}>
                    {plansText}
                  </td>
